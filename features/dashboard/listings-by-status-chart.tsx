...
export function ListingsByStatusChart({
  data,
  onMonthsChange,
  ...props
}: ListingsByStatusChartProps) {
  ...
  const totalByStatus = useMemo(() => {
    const totals = { draft: 0, active: 0, archived: 0 };
    chartData.forEach((d) => {
    ...
    <Card className="w-full" {...props}>
      <CardHeader>
        <CardTitle className="text-base">
          {t("sections.listings_by_status")}
        </CardTitle>
        <CardDescription>
          {t("sections.listings_by_status_description")}
        </CardDescription>
      </CardHeader>
            ...
            >
              <span className="flex items-center gap-2">
                <span
                  ...
                  style={{ backgroundColor: COLORS[status.value] }}
                />
                {status.label}
              </span>
              <span className="font-medium">
                {totalByStatus[status.value as keyof typeof totalByStatus]}
              </span>
            </div>
          ))}
        </div>
