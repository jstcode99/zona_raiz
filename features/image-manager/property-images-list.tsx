...
import {
  SortableImage,
  toSortableImages,
} from "@/application/mappers/property-image.mapper";
...
export function PropertyImageList({ initialImages, propertyId }: Props) {
  const [images, setImages] = useState<SortableImage[]>(() =>
    toSortableImages(initialImages),
  );
  const [deleteId, setDeleteId] = useState<string | null>(null);
  const { t } = useTranslation("images");

  useEffect(() => {
    setImages(toSortableImages(initialImages));
  }, [initialImages]);
  ...
  const { action: setPrimaryAction, isPending: isSettingPrimary } =
    useServerMutation({
  ...
  const { action: updateMetaAction, isPending: isUpdatingMeta } =
    useServerMutation({
  ...
  const { action: deleteImageAction, isPending: isDeleting } =
    useServerMutation({
      action: deletePropertyImageFormAction,
      onSuccess: () => {
        if (deleteId) {
          setImages((prev) =>
            prev.filter((img) => img.id !== undefined && img.id !== deleteId),
          );
        }
        setDeleteId(null);
  ...
  const isPending =
    isReordering || isSettingPrimary || isUpdatingMeta || isDeleting;

  // Reordenar imágenes usando batch action
  const handleReorder = useCallback(
    (newImages: SortableImage[]) => {
      setImages(newImages);
  ...
  const handleMetaChange = useCallback(
    (id: string, data: { alt_text?: string; caption?: string }) => {
